...
class TicketController extends AppBaseController
{
	...
	public function store(CreateTicketRequest $request){
		...
		$assistido =  $this->userRepository->findByUsername($input['username']);
		$refeicao = $this->refeicaoRepository->find($input['refeicao_id']);

		$checkResult = $this->defaultCheckForTicketCreation($assistido,$refeicao);
		if($checkResult == ""){
			...
			Flash::success('Ticket Virtual para  ' . $assistido->name . ' gerado com sucesso. [' . $ticket->getFormattedDataRefeicaoAttribute() . ']');
	...
	private function defaultCheckForTicketCreation($assistido ,$refeicao){
		
		/* verifica se o usuário existe */
		if($assistido == null){
			Flash::error('Ticket Virtual não gerado. O usuário digitado não existe no sistema.');
			return redirect(route('ticket.generate',[$refeicao->id]));
		}
		
		/* verifica se o $assistido possui o auxilio para a $refeicao */
		if(!$this->auxilioRepository->userHasAuxilio($assistido,$refeicao)){
		...
		$todayTicket = $this->ticketRepository->getByAssistidoRefeicaoHoje($assistido, $refeicao);
		if($todayTicket!=null){
			Flash::error('Ticket Virtual não gerado. ' . $assistido->name . ' já realizou essa refeição. [' . $todayTicket->getFormattedDataRefeicaoAttribute() . ']');
			return redirect(route('ticket.generate',[$refeicao->id]));
		}
